module HostedDanger
  class Executor
    ...
    def app_email : String
      ServerConfig.app_email(git_host)
    end
    ...
    def git_fetch_repo
      ...
      exec_cmd("git config --local http.postBuffer 256m", dir)
      exec_cmd("git config --local pack.windowMemory 256m", dir)
      exec_cmd("git config --local pack.packSizeLimit 256m", dir)
