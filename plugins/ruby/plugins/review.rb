module Danger
  # Reviewersへの登録やラベル付与を行います。
  # warnings と errors が存在するかどうかをチェックするため、Dangerfile
  # の最後で呼び出してください。
  #
  # @example 
  #
  # ## request
  #
  #  プルリクの reviewers に登録します
  #
  #   review.request reviewers: ['taro', 'hanako']
  #
  #  依頼と同時にコメントでメンションする場合、 message: "..." を指定します
  #
  #   review.request reviewers: ['taro', 'hanako'], message: "Please review! :pray:"
  #
  #  すでに Reviewer が割り当て済みでも追加で登録したい場合
  #  override: true を指定します
  #
  #   review.request reviewers: ['taro', 'hanako'],
  #                  message: "Please review! :pray:",
  #                  override: true
  #
  #
  # ## apply_inreview_label
  #
  #   warnings も errors もなくWIPでなければ `in review` ラベルをプルリクエストに付けます。
  #   warnings と errorsのチェックのため最後に記述します。
  #
  #    review.apply_inreview_label
  #
  #   ラベル名を変更する場合は label パラメータで指定します。
  #
  #    review.apply_inreview_label label: "review"
  #
  #
  class DangerReview < Plugin
    def request(reviewers: [], message: nil, override: false)
      ...
      #
      # team単位のレビュー依頼機能が入ったら、mergeable?と同じ対応が必要になる
    ...
    #
    # approved_num が指定されているときは Approved した人数をカウントする
    # nil の際は、全員が Approved しているかを確認する
    #
    def mergeable?(approved_num: nil)
      return not_merge "`review#auto_merge`の引数 approved_num を設定してください" if approved_num.nil?
      return not_merge "マージ可能な状態ではありません" unless github.pr_json["mergeable"]
      return not_merge "WIP もしくは DNM です" if wip? || dnm?
      return not_merge "Danger の結果に error もしくは warning があります" unless status_report[:errors].empty? && status_report[:warnings].empty?

      #
      # Review をリクエストされているユーザー
      #
      ...
      if !req_reviewers.kind_of?(Array) && req_reviewers[:users] && req_reviewers[:teams]
        # github.com の方では Reviewer に Team を設定できる関係で API のレスポンスの形式が変わっている。
        # 一応対処しておく。実際に Enterprise に変更が反映されたら動作を確認する必要がある
        # https://developer.github.com/v3/pulls/review_requests/#list-review-requests
      ...
      return not_merge "Reviewer が 設定されていません" if cur_reviews.empty? && req_reviewers.empty?

      #
      # ユーザー名をキーとしたレビュー一覧 ({ 'username' => { [reviews] }, ... })
      #
      cur_reviewers = cur_reviews.group_by { |r| r[:user][:login] }
      
      #
      # cur_reviewers のうち、最新のレビューで Approved しているユーザー (COMMENTED は無視)
      #
      ...
      return not_merge "あと #{approved_num - cur_reviewers_approved.size}人の Reviewer の Approve が必要です" if cur_reviewers_approved.size < approved_num
      return not_merge "Request changes の Reviewer がいます" if cur_reviewers_rejected.size > 0

      statuses = github.api
                   .combined_status(repo, sha)
                   .statuses
                   .reject { |x| x[:context] == 'danger/hosted-danger' }
      return not_merge "Success ではない CI のステータスが存在します" unless statuses.all? { |status| status[:state] == 'success' }
    ...
    def not_merge(reason)
      message "自動マージはブロックされています (#{reason})"
